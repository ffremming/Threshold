...
> **Revision 4 (2026-06-13) — load grounded in the literature.** The per-session **cardio Load** now uses **Edwards' summated heart-rate-zone TRIMP**: `load = Σ (minutes in zone × zone weight)`, weights **Z1×1, Z2×2, Z3×3, Z4×4, Z5×5** (Edwards S., *The Heart Rate Monitor Book*, 1993; validated as a load proxy in Foster et al., *J Strength Cond Res* 2001;15:109–115 — cross-validates with Banister TRIMP r≈0.89 and session-RPE r≈0.67–0.72). This replaces the earlier invented `0.6+0.25·z^1.6` curve. The integer weights are the canonical, most-cited scheme (the literature notes they are linear/arbitrary rather than physiologically derived; a lactate-derived non-linear alternative, Stagno's TRIMPMOD 2007 — 1.25/1.71/2.54/3.61/5.16 — is documented in `constants.js` as an option). **Strength** keeps its volume-load (saturation) track summed onto the same total: a deep-research pass confirmed there is **no validated method to merge resistance volume-load with HR-zone TRIMP** (session-RPE×duration is the only modality-agnostic metric), so the strength→load scale is a documented *pragmatic composite*, not a validated equivalence. Methodology sourced via the deep-research harness (23 sources, 22 verified claims).
